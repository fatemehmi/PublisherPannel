...
function Like(props) {
  const { mutate: addToBookmarks } = useAddToBookmark(props.book_id);
  const { mutate: removeBookmark } = useRemoveBookmark(props.book_id);

  const [liked, setLike] = useState();

  const { isSuccess,isLoading,isError } = useGetIsBookmark(props.book_id, setLike);
